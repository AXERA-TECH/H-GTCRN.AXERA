...
| 模型 | 输入 | 输出 | 端到端 RTF | NPU core RTF |
|------|------|------|:--:|:--:|
| H-GTCRN-core | `feat[1,6,626,257]` | `mask[1,2,626,257]` | ~0.36 | 0.0023 |
...
> **端到端 RTF =（CPU 链路 + NPU）总耗时 / 音频时长**（10s 音频 C++ 约 3.6s，
> AX650 实测）；NPU core RTF 仅神经核部分。CPU 链路（WPE/IVA 整段统计）占绝对
> 主导，与上游一致为离线整段推理。
